...
with_rolling as (
    ...
    select
        ...
        avg(adjusted_close) over (partition by ticker order by date rows between 19  preceding and current row) as ma_20d,
        avg(adjusted_close) over (partition by ticker order by date rows between 49  preceding and current row) as ma_50d,
        avg(adjusted_close) over (partition by ticker order by date rows between 199 preceding and current row) as ma_200d,

        -- Stddev of price (used for Bollinger Bands)
        stddev(adjusted_close) over (partition by ticker order by date rows between 19 preceding and current row) as stddev_20d_price,
...
select
    ...
    case when ma_20d  > 0 then adjusted_close / ma_20d  else null end as price_to_ma_20d,
    case when ma_50d  > 0 then adjusted_close / ma_50d  else null end as price_to_ma_50d,
    case when ma_200d > 0 then adjusted_close / ma_200d else null end as price_to_ma_200d,
    ...
    case when avg_volume_20d > 0 then volume / avg_volume_20d else null end as volume_ratio_20d,
