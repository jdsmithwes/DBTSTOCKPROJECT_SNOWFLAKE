...
last_completed_trading_day as (
    ...
    where
        ...
        and calendar_date not in (select holiday_date from holidays)

)

select
    ds.calendar_date as trading_date
from date_spine ds
cross join last_completed_trading_day lctd
where
    ds.calendar_date <= lctd.last_trading_day
    and dayofweek(ds.calendar_date) not in (0, 6)
    and ds.calendar_date not in (select holiday_date from holidays)
